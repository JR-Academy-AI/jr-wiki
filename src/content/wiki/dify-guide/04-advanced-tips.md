...
在 Claude Desktop 或 Cursor 里调用你在 Dify 搭建的 RAG 知识库：
...
```json
{
  "mcpServers": {
    "dify-rag": {
      "command": "uv",
      "args": ["--directory", "/path/to/dify-mcp-server", "run", "dify_mcp_server"],
      "env": {
        "DIFY_BASE_URL": "https://dify.yourcompany.com/v1",
        "DIFY_APP_SKS": "app-sk-your-rag-app-key"
      }
    }
  }
}
...
反过来，把 Dify 应用暴露成 MCP Server 也只需在发布设置里开启——外部客户端通过标准 MCP 协议调用你的 Workflow，不用写适配代码。
...
label:
  ...
  zh_Hans: 订单查询
description:
  en_US: Query order status by order ID
  zh_Hans: 根据订单号查询物流状态
```

```python
# tools/order_lookup.py
from dify_plugin import Tool, ToolProviderCredentialValidationError

class OrderLookupTool(Tool):
    def _invoke(self, tool_parameters: dict) -> str:
        order_id = tool_parameters.get("order_id")
        # 调你自己的订单 API
        resp = requests.get(f"https://api.internal/orders/{order_id}")
